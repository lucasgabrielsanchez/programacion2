...
namespace Entidades_2017
{
    ...
    public class Changuito
    {
        ...
        public static Changuito operator +(Changuito c, Producto p)
        {
            ...
            {
                //if (item == p)
                  if (item.Equals(p)) //Comparo la igualdad con el metodo virtual Equals sobreescrito
                    return c;         //en mi clase Producto
            }

            if (c._productos.Count < c._espacioDisponible)//solo agrego elementos a la lista mientras el objeto a agregar
            {                                             //no supere el límite de espacio disponible que elijo en el main
                c._productos.Add(p);
            }
